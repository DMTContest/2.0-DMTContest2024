...
public class OptionAG310 : MonoBehaviour
{

    public TimeAG310 timeAG11;

    private bool isChoose = false;
    private OptionAG310 AOption; // 00选项的脚本引用
    private OptionAG310 BOption; // 01选项的脚本引用
    private OptionAG310 COption; // 10选项的脚本引用
    private OptionAG310 DOption; // 11选项的脚本引用
    ...
    private void Start()
    {
        ...
        AOption = OA.GetComponent<OptionAG310>();

        GameObject OB = GameObject.Find("Btn_1");

        BbuttonImage = OB.GetComponent<Image>();
        BOption = OB.GetComponent<OptionAG310>();

        GameObject OC = GameObject.Find("Btn_3");
        COption = OC.GetComponent<OptionAG310>();

        GameObject OD = GameObject.Find("Btn_4");
        DOption = OD.GetComponent<OptionAG310>();
    ...
    private void CheckAnswer()
    {
            ...
            DOption.isChoose == false)
        {
            Debug.Log("答对啦!");
            AOption.correct = 1;
            SceneManager.LoadScene("AG3Final");
        }
        else
        {
            Debug.Log("答错啦!");
